...
export class ObsidianGitSettingsTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl)
            ...
            .setDesc(
                'Set the style for the diff view. Note that the actual diff in "Split" mode is not generated by Git, but the editor itself instead so it may differ from the diff generated by Git. One advantage of this is that you can edit the text in that view. Mobile always uses the unified view.'
            )
            .addDropdown((dropdown) => {
        ...
        new Setting(containerEl)
            .setName("Submodules, native Git binary, and environment")
            .setDesc(
                "The wasm-git engine does not support submodules, a custom Git binary, or extra environment variables. SSH remotes are not supported; use HTTPS with a token or password."
            );
    ...
    private addLineAuthorInfoSettings() {
        ...
        if (
            ...
            baseLineAuthorInfoSetting
                .setDesc("Line authoring is unavailable until Git is ready.")
                .setDisabled(true);
        }
